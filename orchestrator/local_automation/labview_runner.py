"""LabVIEW RvR Wireless v2.03 GUI automation runner.

Drives the 19-step wizard flow through to test start using pyautogui
for all mouse/keyboard interactions.  LabVIEW custom controls do NOT
respond to Win32 keybd_event, SendInput, or WM_CHAR -- pyautogui is
the only reliable input method.
...
Key architectural decisions (learned from calibration):
  ...
  - LabVIEW VIs don't scale controls with window size
...
Multi-band support:
  ...
  - run_labview_flow can be called per-band; the multi-band loop
    calls it once for each band after the previous test finishes
"""
...
def step_06_select_ap(hwnd: int, cfg: RunConfig, ad: str) -> StepResult:
    ...
    if popup:
        ...
        if hit:
            ...
            logger.warning(
                "OCR UNAVAILABLE — using folder-index nav for AP %r.",
                cfg.ap_name,
                extra={"action": "select_ap", "step": "no_ocr_folder"})
            _select_list_item_by_folder_index(
                popup, AP_FOLDER, cfg.ap_name, ad, 6, "ap")
        else:
            _screenshot(popup, ad, 6, "ap_ocr_failed")
            logger.error("OCR search for AP %r FAILED — aborting step "
                         "(no blind-scroll fallback)", cfg.ap_name,
                         extra={"action": "select_ap", "step": "ocr_fail_safe"})
            return StepResult("select_ap", False, time.monotonic() - t0,
                              error=f"AP {cfg.ap_name!r} not found via OCR")
...
def step_07_use_last_ap(hwnd: int, cfg: RunConfig, ad: str) -> StepResult:
    ...
    return StepResult("use_last_ap", False, time.monotonic() - t0, ss,
                      error=f"Screen did not advance after {MAX_WAIT:.0f}s")
...
def step_08_select_client(hwnd: int, cfg: RunConfig, ad: str) -> StepResult:
    ...
    if popup:
        ...
        if hit:
            ...
            logger.error("OCR search for client %r FAILED — aborting step "
                         "(no blind-scroll fallback)", cfg.client_name,
                         extra={"action": "select_client",
                                "step": "ocr_fail_safe"})
            return StepResult("select_client", False, time.monotonic() - t0,
                              error=f"Client {cfg.client_name!r} not found via OCR")
...
def run_labview_flow(
    ...
    skip_to_step: int = 0,
    dry_run: bool = False,
    post_step_hooks: dict[int, Callable] | None = None,
) -> RunReport:
    global _DRY_RUN, _DRY_RUN_IMG, _DRY_RUN_HWND
    _DRY_RUN = dry_run
    ...
    report = RunReport(started_at=ts, config=asdict(cfg))

    logger.info("LabVIEW runner started (band=%s, mode=%s, user_info=%s, dry_run=%s)",
    ...
    # Close orphaned sub-VI windows from previous failed runs
    for h, t, w, ht, r in _enum_lv_windows():
    ...
    fcfg, initial_files = _build_finish_config(cfg)
    ...
    if hwnd is None and not dry_run:
        ...
        _save_report(report, ad)
        return report

    for i, step_fn in enumerate(STEP_SEQUENCE):
        if i < skip_to_step:
            continue

        _check_stop()

        # Dry-run: capture a fresh screenshot for annotation
        if dry_run and hwnd:
            try:
                _DRY_RUN_IMG = capture_window(hwnd)
            except Exception:
                _DRY_RUN_IMG = None
            _DRY_RUN_HWND = hwnd
        ...
        for attempt in range(MAX_STEP_RETRIES + 1):
            try:
                ...
                if attempt < MAX_STEP_RETRIES:
                    logger.warning(
                        "Step %d/%d %s failed (attempt %d/%d), "
                        "diagnosing before retry...",
                        i, len(STEP_SEQUENCE) - 1, result.name,
                        attempt + 1, MAX_STEP_RETRIES + 1,
                        extra={"action": "labview_step", "step": "retry"})

                    diagnosis = _diagnose_failure(hwnd, result.name, ad)
                    recovered_hwnd = _recover_from_failure(hwnd, diagnosis)
                    if recovered_hwnd:
                        hwnd = recovered_hwnd

                    backoff = RETRY_DELAY_SEC * (attempt + 1)
                    time.sleep(backoff)

            except EmergencyStopError:
                logger.critical("EMERGENCY STOP triggered during step %d (%s)",
                                i, step_fn.__name__,
                                extra={"action": "emergency_stop", "step": step_fn.__name__})
                last_result = StepResult(
                    step_fn.__name__, False,
                    error="Emergency stop triggered")
                report.steps.append(asdict(last_result))
                ...
                logger.error("Step %d exception (attempt %d): %s",
                             i, attempt + 1, exc,
                             extra={"action": "labview_step", "step": "exception"})
                try:
                    vi = _find_active_vi() or hwnd
                    if vi:
                        _screenshot(vi, ad, i, f"exception_attempt{attempt}")
                except Exception:
                    pass

                last_result = StepResult(
                    step_fn.__name__, False, error=str(exc),
                    detail={"traceback": tb})

                if attempt < MAX_STEP_RETRIES:
                    diagnosis = _diagnose_failure(hwnd, step_fn.__name__, ad)
                    recovered_hwnd = _recover_from_failure(hwnd, diagnosis)
                    if recovered_hwnd:
                        hwnd = recovered_hwnd
                    backoff = RETRY_DELAY_SEC * (attempt + 1)
        ...
        # Save annotated dry-run screenshot after each step
        if dry_run:
            _save_dry_run_screenshot(ad, i, step_fn.__name__)

        report.steps.append(asdict(last_result))
        logger.info("Step %d/%d %s: %s (%.1fs)%s",
                     i, len(STEP_SEQUENCE) - 1, last_result.name,
                     "OK" if last_result.success else "FAIL", last_result.elapsed_sec,
                     "" if step_success else " [EXHAUSTED RETRIES]",
                     extra={"action": "labview_step", "step": last_result.name})

        if step_success and post_step_hooks and i in post_step_hooks:
            hook_fn = post_step_hooks[i]
            try:
                logger.info("Running post-step hook for step %d (%s)",
                            i, step_fn.__name__,
                            extra={"action": "post_step_hook", "step": str(i)})
                hook_fn(cfg, ad)
            except Exception as exc:
                logger.error("Post-step hook for step %d failed: %s", i, exc,
                             extra={"action": "post_step_hook", "step": "error"})

        if not step_success:
            report.error = f"Step {last_result.name} failed: {last_result.error}"
            break

    if not report.error:
        report.success = True
        logger.info("All wizard steps completed - test is starting",
                     extra={"action": "labview_runner", "step": "wizard_done"})

        if dry_run:
            logger.info("Dry-run complete: annotated screenshots in %s", ad,
                         extra={"action": "labview_runner", "step": "dry_run_done"})
        else:
            if fcfg.ui_hwnd == 0:
                fcfg.ui_hwnd = _find_active_vi() or hwnd or 0
            fr = wait_for_finish(fcfg, artifacts_dir=ad, initial_files=initial_files)
            report.finish_result = asdict(fr)
            if fr.timed_out:
                report.error = "Test timed out"
                report.success = False
            elif fr.finished:
                logger.info("Test finished: method=%s detail=%s",
                             fr.method, fr.detail,
...
def run_all_bands(
    ...
    """Run the LabVIEW flow for each band sequentially.

    After each test finishes (detected via PDF in D:\\480\\LOG\\RBU),
    the program returns to the main screen. The next band starts
    from step_00 (attach) through step_18 (final_start).

    *post_step_hooks* is forwarded to each ``run_labview_flow`` call
    (e.g. WiFi connect hook after step 16).
    """
    ...
    for idx, band in enumerate(bands):
        logger.info("=== Multi-band: starting band %d/%d: %s ===",
        ...
        report = run_labview_flow(
            cfg, artifacts_base=artifacts_base, dry_run=dry_run,
            post_step_hooks=post_step_hooks,
        )
        reports.append(report)

        if not report.success:
            logger.error("Band %s FAILED: %s", band, report.error,
                          extra={"action": "multi_band", "step": "band_fail"})
            break

        logger.info("Band %s completed successfully", band,
    ...
    logger.info("Multi-band run complete: %d/%d passed",
...
def main():
    ...
    parser.add_argument("--mode", default="BW20", choices=list(BW_MODE_NAV.keys()))
