...
BAND_DEFAULT_SECURITY = {"2.4G": "wpa2", "5G": "wpa2", "6G": "wpa3"}
...
def _build_router_cfg_for_iteration(
    ...
    for band in detected_bands:
        ...
        bands[band] = BandWifiConfig(
            ...
            security=BAND_DEFAULT_SECURITY.get(band, "wpa2"),
        )
...
async def run_sweep(sweep_wf: SweepWorkflow) -> dict[str, Any]:
    ...
    router_control_url = sweep_wf.router.router_control_url or DEFAULT_ROUTER_CONTROL_URL

    logger.info("Detecting available bands via router-control service at %s",
                router_control_url,
    ...
    for idx, (band, channel) in enumerate(iterations):
        ...
        if iter_success:
            ...
            connect_security = BAND_DEFAULT_SECURITY.get(band, "wpa2")
            result = await step_connect_workers(
                workers=workers,
                ssid=connect_ssid,
                password=sweep.password,
                security=connect_security,
                artifacts_dir=iter_dir,
            )
